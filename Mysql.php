<?php
	class Mysql{
		...
		function __construct($host, $user, $pass, $db, $port){
			self::$connection_info = array('host' => $host, 'user' => $user, 'pass' => $pass, 'db' => $db, 'port' => $port);
		}
		...
		static private function connection(){
			if(!is_resource(self::$link) || empty(self::$link)){
				if(($link = mysqli_connect(self::$connection_info['host'], self::$connection_info['user'], self::$connection_info['pass'], self::$connection_info['db'], self::$connection_info['port'])) && mysqli_select_db($link, self::$connection_info['db'])){
					self::$link = $link;
					mysqli_set_charset(self::$link, 'utf8');
				}else{
					throw new Exception('Could not connect to MySQL database. ' . mysqli_connect_error());
				}
			}
			return self::$link;
		}
